...
int main()
{
    ...
    for (uint32_t i = 0; i<deviceCount; ++i)
    {
        ...
        printf("    Max Compute WG cnt:  %u x %u x %u\n", device_props.limits.maxComputeWorkGroupCount[0], device_props.limits.maxComputeWorkGroupCount[1], device_props.limits.maxComputeWorkGroupCount[2]);
        printf("    Max Compute WG size: %u x %u x %u\n", device_props.limits.maxComputeWorkGroupSize[0], device_props.limits.maxComputeWorkGroupSize[1], device_props.limits.maxComputeWorkGroupSize[2]);
        printf("    Max Compute SharedM: %u\n", device_props.limits.maxComputeSharedMemorySize);
        printf("    Max Framebuf Dims:   %u x %u x %u\n", device_props.limits.maxFramebufferWidth, device_props.limits.maxFramebufferHeight, device_props.limits.maxFramebufferLayers);
